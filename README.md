...
This package removes the old `GetEventDebugID()` listener callback because the current CS2 `IGameEventListener2` interface only requires `FireGameEvent()`.

It also defines the missing `INTERFACEVERSION_GAMEEVENTSMANAGER2` compatibility macro as `GAMEEVENTSMANAGER002` for the current public CS2 HL2SDK, and defines `NOMINMAX` before including `windows.h` to avoid MSVC `std::min` macro conflicts.
