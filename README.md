...
- Corrected Source2Server vfunc 91 game-event manager fallback to use the required `raw - 8` adjustment.
- Removed the risky SourceHook `FireEvent` hook; the plugin now uses only the normal `weapon_fire` listener.
- Loads gamedata/log files from MetaMod's absolute `game/csgo` directory instead of the process current working directory.
- Installs the particle detour lazily on the first silenced `weapon_fire` instead of during server startup.
