...
This build avoids the old `IGameEventManager2` / `Source2Server` vfunc path that crashed on startup. It hooks CS2's `IGameEventSystem::PostEventAbstract`, serializes `GE_FireBullets` with `INetworkMessages`, parses the protobuf bytes manually, and blocks only messages for silenced weapon item definitions:
...
This version does not include or require `te.pb.h`; it avoids generated protobuf headers so the GitHub Actions build can use the public HL2SDK CS2 branch directly.
