...
SS-12: safe loader/event-interface fix. This version uses GAMEEVENTSMANAGER001/002 lookup without the unsafe Source2Server vfunc fallback, reads gamedata from the real game/csgo directory, and keeps the tracer-only particle detour.
...
The Windows GitHub runner previously generated `SilencedTracerBlocker.vdf` with backslashes in the plugin path. This build forces forward slashes so the generated VDF should contain exactly:

```text
"Metamod Plugin"
{
    "alias" "stb"
    "file"  "addons/SilencedTracerBlocker/bin/win64/SilencedTracerBlocker"
}
```

If the plugin still does not appear in `meta list`, test it manually in server console:

```text
meta load addons/SilencedTracerBlocker/bin/win64/SilencedTracerBlocker
```

That command should print the real load error if the VDF is not the problem.

## SS-13 change

This version no longer depends on `IGameEventManager2`, because some CS2 dedicated servers do not expose `GAMEEVENTSMANAGER001/002` to MetaMod plugins.

It hooks `GameEventSystemServerV001::PostEventAbstract` directly and watches only `CMsgTEFireBullets` messages. If the message belongs to a silenced weapon item definition, it is blocked before BetterFeedback can replace the shared tracer particle.

Blocked item definitions:

- `60` M4A1-S
- `61` USP-S
- `23` MP5-SD

It does not hook or block `bullet_impact`, `player_hurt`, blood, decals, ricochet, taser, or non-silenced weapon messages.
