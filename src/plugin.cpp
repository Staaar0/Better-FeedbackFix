...
struct FireBulletsFields
{
    int32_t itemDefIndex = 0;
    int32_t weaponId = 0;
    int32_t soundType = 0;
    int32_t mode = 0;
...
bool ReadVarint(const uint8_t *data, std::size_t size, std::size_t &pos, uint64_t &out)
{
    out = 0;
    int shift = 0;
...
bool SkipLengthDelimited(const uint8_t *data, std::size_t size, std::size_t &pos)
{
    uint64_t length = 0;
    if (!ReadVarint(data, size, pos, length))
        return false;
...
bool ParseFireBulletsFields(const uint8_t *data, std::size_t size, FireBulletsFields &fields)
{
    std::size_t pos = 0;
    ...
    {
        uint64_t key = 0;
        if (!ReadVarint(data, size, pos, key))
            return false;
        ...
        switch (wireType)
        {
            case 0: // varint
            {
                ...
                switch (fieldNumber)
                {
                    case 3:  fields.weaponId = static_cast<int32_t>(value); break;
                    case 4:  fields.mode = static_cast<int32_t>(value); break;
                    case 10: fields.soundType = static_cast<int32_t>(value); break;
                    case 11:
                        fields.itemDefIndex = static_cast<int32_t>(value);
                        fields.hasItemDefIndex = true;
                        break;
                    default: break;
                }
                break;
            ...
            case 2: // length-delimited
                if (!SkipLengthDelimited(data, size, pos))
                    return false;
                break;
...
bool ExtractFireBulletsFields(INetworkMessageInternal *pEvent, const CNetMessage *pData, FireBulletsFields &fields)
{
    if (!g_NetworkMessages || !pEvent || !pData)
        return false;

    alignas(4) uint8_t buffer[4096]{};
    bf_write writer("SilencedTracerBlocker.FireBullets", buffer, static_cast<int>(sizeof(buffer)));

    if (!g_NetworkMessages->SerializeAbstract(writer, pEvent, pData) || writer.IsOverflowed())
        return false;

    const int bytesWritten = writer.GetNumBytesWritten();
    if (bytesWritten <= 0 || bytesWritten > static_cast<int>(sizeof(buffer)))
        return false;

    return ParseFireBulletsFields(buffer, static_cast<std::size_t>(bytesWritten), fields);
}
...
INetworkMessages *AcquireNetworkMessages(ISmmAPI *ismm)
{
    if (!ismm)
        return nullptr;
    ...
    for (const auto &entry : factories)
    {
        ...
        auto *networkMessages = reinterpret_cast<INetworkMessages *>(
            ismm->VInterfaceMatch(entry.factory, NETWORKMESSAGES_INTERFACE_VERSION, 0));

        if (networkMessages)
        {
            Log("[STB] Found %s from %s factory.\n", NETWORKMESSAGES_INTERFACE_VERSION, entry.name);
...
bool SilencedTracerBlocker::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
    ...
    g_NetworkMessages = AcquireNetworkMessages(ismm);
    if (!g_NetworkMessages)
    {
        Log("[STB] ERROR: Could not find %s. Plugin will load safely but cannot inspect FireBullets payloads.\n",
                                           ...
                                           NetChannelBufType_t bufType)
{
    ...
    FireBulletsFields fields;
    if (!ExtractFireBulletsFields(pEvent, pData, fields))
    {
        const int seen = g_DebugSeenFireBullets++;
        if (seen < 32)
            Log("[STB] GE_FireBullets seen but payload parse failed. clients=%d size=%lu network_messages=%s\n",
                nClientCount, nSize, g_NetworkMessages ? "yes" : "no");
        RETURN_META(MRES_IGNORED);
    }

    const int32_t itemDefIndex = fields.itemDefIndex;
    const int seen = g_DebugSeenFireBullets++;
    if (seen < 32)
    {
        Log("[STB] GE_FireBullets seen: item_def_index=%d weapon_id=%d sound_type=%d mode=%d has_item_def=%s clients=%d\n",
            itemDefIndex,
            fields.weaponId,
            fields.soundType,
            fields.mode,
            fields.hasItemDefIndex ? "true" : "false",
            nClientCount);
    }

    if (fields.hasItemDefIndex && IsSilencedItemDefinition(itemDefIndex))
    {
