...
constexpr size_t kExtraBaseOffset = 0x98;
constexpr size_t kObjectEndOffset = 0xA0;
...
void ZeroRangeExceptSound(unsigned char *data, size_t size)
{
    if (!data || size <= kWeaponIdOffset)
        return;

    const size_t end = (size < kObjectEndOffset) ? size : kObjectEndOffset;
    for (size_t i = kWeaponIdOffset; i < end; ++i)
    {
        const bool keepSoundType = i >= kSoundTypeOffset && i < kSoundTypeOffset + sizeof(int32_t);
        const bool keepSoundDsp = i >= kSoundDspOffset && i < kSoundDspOffset + sizeof(int32_t);
        if (!keepSoundType && !keepSoundDsp)
            data[i] = 0;
    }
}

void ClearTracerPresenceBits(unsigned char *data, size_t size)
{
    if (!data || size < kWeaponIdOffset)
        return;

    // Search only the small pre-field area for the generated protobuf has-bits
    // word. It should be a compact value, not a pointer, and it must contain
    // the FireBullets-specific scalar bits we already verified from this shot.
    for (size_t offset = 0x20; offset + sizeof(uint32_t) <= kWeaponIdOffset; offset += sizeof(uint32_t))
    {
        uint32_t bits = 0;
        if (!ReadUInt32(data, size, offset, bits))
            continue;

        if ((bits & kHasBitWeaponId) == 0 ||
            (bits & kHasBitSoundType) == 0 ||
            (bits & kHasBitItemDef) == 0 ||
            (bits & ~kAllExpectedFieldBitsMask) != 0)
        {
            continue;
        }

        const uint32_t kept = bits & kKeepSoundFieldBits;
        WriteUInt32(data, size, offset, kept);

        // The generated cached-size word is normally immediately after the
        // has-bits word. Clearing it avoids stale protobuf serialization size.
        WriteInt32(data, size, offset + sizeof(uint32_t), 0);
        return;
    }
}
                            ...
                            unsigned long messageSize)
{
    if (IsFireBullets(networkMessage) && IsSuppressedFireBullets(messageData, messageSize))
    {
        ...
        // Keep the FireBullets event alive so the CS2 weapon shot sound remains,
        // but strip every known tracer-driving scalar/flag from the live protobuf
        // object. Sound fields are intentionally preserved.
        ClearTracerPresenceBits(bytes, size);
        ZeroRangeExceptSound(bytes, size);

        WriteInt32(bytes, size, kWeaponIdOffset, 0);
        WriteInt32(bytes, size, kModeOffset, 0);
        WriteInt32(bytes, size, kSeedOffset, 0);
        WriteInt32(bytes, size, kPlayerOffset, 0);
        WriteFloat(bytes, size, kInaccuracyOffset, 0.0f);
        WriteInt32(bytes, size, kRecoilIndexOffset, 0);
        WriteFloat(bytes, size, kSpreadOffset, 0.0f);
        WriteInt32(bytes, size, kItemDefOffset, 0);
        WriteInt32(bytes, size, kNumBulletsRemainingOffset, 0);
        WriteInt32(bytes, size, kAttackTypeOffset, 0);
        WriteUInt8(bytes, size, kPlayerInAirOffset, 0);
        WriteUInt8(bytes, size, kPlayerScopedOffset, 0);
        WriteInt32(bytes, size, kTickOffset, 0);

        // The repeated Extra block is after the scalar fields in the current
        // CS2 layout. Clearing this small tail removes extra visual/tracer
        // metadata while keeping the sound fields above.
        if (size > kExtraBaseOffset)
            std::memset(bytes + kExtraBaseOffset, 0, ((size < kObjectEndOffset) ? size : kObjectEndOffset) - kExtraBaseOffset);
