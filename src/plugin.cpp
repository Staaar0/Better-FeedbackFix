                            ...
                            unsigned long messageSize)
{
    if (IsFireBullets(networkMessage) && IsSuppressedFireBullets(messageData, messageSize))
    {
        ...
        int32_t oldWeaponId = 0;
        int32_t oldMode = 0;
        int32_t oldSeed = 0;
        int32_t oldPlayer = 0;
        int32_t oldRecoilIndex = 0;
        int32_t oldItemDef = 0;
        int32_t oldRemaining = 0;
        float oldInaccuracy = 0.0f;
        float oldSpread = 0.0f;

        const bool restoreWeaponId = ReadInt32(bytes, size, kWeaponIdOffset, oldWeaponId);
        const bool restoreMode = ReadInt32(bytes, size, kModeOffset, oldMode);
        const bool restoreSeed = ReadInt32(bytes, size, kSeedOffset, oldSeed);
        const bool restorePlayer = ReadInt32(bytes, size, kPlayerOffset, oldPlayer);
        const bool restoreInaccuracy = ReadFloat(bytes, size, kInaccuracyOffset, oldInaccuracy);
        const bool restoreRecoilIndex = ReadInt32(bytes, size, kRecoilIndexOffset, oldRecoilIndex);
        const bool restoreSpread = ReadFloat(bytes, size, kSpreadOffset, oldSpread);
        const bool restoreItemDef = ReadInt32(bytes, size, kItemDefOffset, oldItemDef);
        const bool restoreRemaining = ReadInt32(bytes, size, kNumBulletsRemainingOffset, oldRemaining);

        // Sound-only pass: keep origin, sound_type and sound_dsp_effect intact,
        // but clear the fields used by the client tracer/build-feedback path.
        WriteInt32(bytes, size, kWeaponIdOffset, 0);
        WriteInt32(bytes, size, kModeOffset, -1);
        ...
        if (restoreWeaponId)
            WriteInt32(bytes, size, kWeaponIdOffset, oldWeaponId);
        if (restoreMode)
            WriteInt32(bytes, size, kModeOffset, oldMode);
        if (restoreSeed)
            WriteInt32(bytes, size, kSeedOffset, oldSeed);
        if (restorePlayer)
            WriteInt32(bytes, size, kPlayerOffset, oldPlayer);
        if (restoreInaccuracy)
            WriteFloat(bytes, size, kInaccuracyOffset, oldInaccuracy);
        if (restoreRecoilIndex)
            WriteInt32(bytes, size, kRecoilIndexOffset, oldRecoilIndex);
        if (restoreSpread)
            WriteFloat(bytes, size, kSpreadOffset, oldSpread);
        if (restoreItemDef)
            WriteInt32(bytes, size, kItemDefOffset, oldItemDef);
        if (restoreRemaining)
            WriteInt32(bytes, size, kNumBulletsRemainingOffset, oldRemaining);
        return;
    }
