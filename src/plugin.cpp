...
void ClearTracerExtraTail(unsigned char *data, size_t size)
{
    if (!data || size <= kExtraBaseOffset)
        return;

    const size_t end = (size < kObjectEndOffset) ? size : kObjectEndOffset;
    if (end > kExtraBaseOffset)
        std::memset(data + kExtraBaseOffset, 0, end - kExtraBaseOffset);
}
                            ...
                            unsigned long messageSize)
{
    if (IsFireBullets(networkMessage) && IsSuppressedFireBullets(messageData, messageSize))
    {
        ...
        // Keep the full FireBullets scalar data for CS2's weapon shot sound.
        // Only clear the small generated-protobuf tail that was found to drive
        // BetterFeedback's visible custom tracer for suppressed shots.
        ClearTracerExtraTail(bytes, size);
