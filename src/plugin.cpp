...
// normal engine factory on all builds. Several working CS2 MetaMod plugins
// obtain the game-event API from ISource2Server vfunc 91 instead, so this
// plugin tries the old factory names first and then falls back to that path.
...
IGameEventManager2 *TryAcquireGameEventManager(ISmmAPI *ismm)
{
    ...
    auto *manager = reinterpret_cast<IGameEventManager2 *>(CallVFunc<void *, 91>(g_Source2Server));
    if (manager)
    {
        Log("[STB] IGameEventManager2 acquired from ISource2Server vfunc 91.\n");
        return manager;
    }
