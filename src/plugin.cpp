...
#else
    if (!ProtectSlot(slot, PROT_READ | PROT_WRITE))
        return false;
    *slot = hook;
    ProtectSlot(slot, PROT_READ);
#endif
...
#else
    if (ProtectSlot(g_HookedSlot, PROT_READ | PROT_WRITE))
    {
        *g_HookedSlot = g_OriginalSlotValue;
        ProtectSlot(g_HookedSlot, PROT_READ);
    }
#endif
...
{
    return "https://github.com/Staaar0/BetterFeedbackFix";
}
