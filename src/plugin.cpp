...
// Some current CS2 builds expose GAMEEVENTSMANAGER001 instead of 002.
...
void Log(const char *format, ...)
{
    ...
    // Write near the current process and into the real game/csgo folder.
    ...
    // Extra fallbacks for unusual host panels.
...
IGameEventManager2 *AcquireGameEventManager(ISmmAPI *ismm)
{
    if (!ismm)
        return nullptr;
    ...
    for (const auto &entry : factories)
    {
        ...
        for (const char *version : versions)
        {
            ...
            auto *manager = reinterpret_cast<IGameEventManager2 *>(
                ismm->VInterfaceMatch(entry.factory, version, 0));

            if (manager)
            {
                Log("[STB] Found IGameEventManager2 as %s from %s factory.\n", version, entry.factoryName);
                return manager;
            }
        }
    }
...
bool SilencedTracerBlocker::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
    ...
    g_GameEvents = AcquireGameEventManager(ismm);
    if (!g_GameEvents)
    {
        Log("[STB] ERROR: Could not find IGameEventManager2 as GAMEEVENTSMANAGER001 or 002. Plugin loaded safely but cannot see weapon_fire.\n");
        return true;
    }
...
{
    const std::string path1 = GamePath("addons/SilencedTracerBlocker/gamedata/silenced_tracer_blocker.games.txt");
    std::ifstream file(path1);

    if (!file.is_open())
    {
        // Also try relative paths for unusual host panels.
        file.open("addons/SilencedTracerBlocker/gamedata/silenced_tracer_blocker.games.txt");
    }

    if (!file.is_open())
    {
        file.open("game/csgo/addons/SilencedTracerBlocker/gamedata/silenced_tracer_blocker.games.txt");
    }

    if (!file.is_open())
    {
        std::snprintf(error, maxlen, "Could not open gamedata/silenced_tracer_blocker.games.txt. Tried %s", path1.c_str());
        return false;
    }
