...
void ClearExtraTypeOnly(unsigned char *data, size_t size)
{
    ...
    if (!IsWritableMemory(extra, 0x50))
        return;

    // Keep the Extra object pointer valid for CS2's surface impact path.
    // Only clear the small Extra.type value that drives BetterFeedback's tracer.
    // Do not scan or modify protobuf header/has-bits here; touching the wrong
    // word can corrupt the generated protobuf object and crash the server.
    constexpr size_t kCandidateOffsets[] = {0x44, 0x40, 0x3C, 0x38, 0x34};
    for (size_t offset : kCandidateOffsets)
    {
        int32_t value = 0;
        if (!SafeReadInt32(extra, offset, value))
            continue;
