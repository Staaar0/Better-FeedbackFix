...
bool ShouldHideTracer(const CNetMessage *message, unsigned long messageSize)
{
    if (!message)
        return false;
    ...
    int32_t mode = 0;
    int32_t soundType = 0;
    ReadInt32(data, size, kModeOffset, mode);
    ReadInt32(data, size, kSoundTypeOffset, soundType);

    // Suppressed USP-S/M4A1-S FireBullets currently carries mode=1 and
    // silenced sound_type=9. When the suppressor is detached, mode/sound_type
    // change and the message is passed through untouched so tracers show again.
    return mode != 0 || soundType == 9;
}
...
{
    if (IsFireBullets(networkMessage) && ShouldHideTracer(messageData, messageSize))
    {
        ...
        // Keep the FireBullets message so clients still receive shot audio and
        // normal impacts. The visible tracer path depends on the fire mode /
        // weapon identity, while suppressed audio is kept by leaving sound_type
        // and sound_dsp_effect untouched.
